...
export function createProjectImage(title, accent = '#0F53B8') {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" fill="none">
      <defs>
        <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stop-color="#931521" />
          <stop offset="100%" stop-color="${accent}" />
        </linearGradient>
        <filter id="blur"><feGaussianBlur stdDeviation="24" /></filter>
      </defs>
      <rect width="800" height="600" rx="36" fill="url(#g)" />
      <circle cx="650" cy="120" r="140" fill="#0F53B8" opacity="0.18" filter="url(#blur)" />
      ...
      <path d="M130 420C200 300 305 264 410 292C495 315 562 384 680 258" stroke="#0F53B8" stroke-width="16" stroke-linecap="round" />
      <path d="M130 356C210 245 326 218 422 240C514 262 587 331 680 214" stroke="#931521" stroke-width="12" stroke-linecap="round" opacity="0.88" />
...
export function createAvatarImage(name, accent = '#0F53B8') {
  const initials = name
    .split(' ')
    .map((part) => part[0])
    .slice(0, 2)
